...
for model_cfg in model_configs:
    ...
    for item in data:
        ...
        for est in data[item]:
            if data_raw[item][0]['label']!=0:
                totalPred += 1
                results['total'] += 1
                if (est['Correct'] and data_raw[item][0]['label'] == 1) or (not est['Correct'] and data_raw[item][0]['label'] == 2):
                    totalCorrect += 1
                    results['correct'] += 1
    
    print(f"{json_name} has {totalCorrect} correct pred out of {totalPred}, ratio: {(totalCorrect/totalPred * 100):.1f}\n")
    # print(f"{(totalCorrect/totalPred * 100):.1f}")
